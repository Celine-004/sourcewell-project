...
class RiskDashboard:
    ...
    def run_risk_assessment(self, patient_data, selected_calculators: Optional[List[str]] = None) -> Dict[str, Any]:
        """Run risk calculators, optionally filtered by selection"""
        try:
            # Run all calculators
            results = self.runner.run_all_assessments(patient_data)

            # Filter to selected calculators if specified
            if selected_calculators and results.get('success') and results.get('results'):
                filtered = {
                    k: v for k, v in results['results'].items()
                    if k in selected_calculators
                }
    ...
    def _display_framingham_result(self, data: Any):
        """Display Framingham results with AHA/ACC risk tiers"""
        ten_year_risk = data.ten_year_risk_percentage if hasattr(data, 'ten_year_risk_percentage') else 0
        bp_category = data.bp_category if hasattr(data, 'bp_category') else 'Unknown'

        # AHA/ACC 2018 four-tier classification
        if ten_year_risk < 5:
            risk_level = "Low"
        ...
        progress = min(ten_year_risk / 30, 1.0)
        st.progress(progress)
    ...
    def create_risk_visualization(self, results: Dict[str, Any]):
        ...
        # Collect risk data for percentage-based calculators
        risk_data = []
        ...
        if risk_data:
            # Bar chart comparing risk percentages
            fig = px.bar(
                risk_data,
                x='Calculator',
                y='10-Year Risk %',
                color='Category',
                title='Estimated 10-Year Risk: Diabetes vs. Cardiovascular',
                color_discrete_map={
                    'Metabolic': '#FF6B6B',
            ...
            for i, data in enumerate(risk_data):
                with cols[i]:
                    fig_gauge = go.Figure(go.Indicator(
                        ...
                        gauge={
                            'axis': {'range': [0, 50]},
                            'bar': {'color': "darkred" if data['10-Year Risk %'] >= 20 else "orange" if data['10-Year Risk %'] >= 7.5 else "gold" if data['10-Year Risk %'] >= 5 else "green"},
                            'steps': [
                                {'range': [0, 5], 'color': "#d4edda"},
                                {'range': [5, 7.5], 'color': "#fff3cd"},
                                {'range': [7.5, 20], 'color': "#ffe0b2"},
    ...
    def _display_risk_factors_summary(self, calculator_results: Dict[str, Any]):
        ...
        for calc_name in ['diabetes', 'cardiovascular', 'colorectal_screening']:
            ...
            if calc_name == 'cardiovascular' and hasattr(calc_result, 'ten_year_risk_percentage'):
                ...
                elif calc_result.ten_year_risk_percentage >= 7.5:
                    risk_factors.append("Intermediate cardiovascular risk (7.5-20%)")
        ...
        if risk_factors:
            ...
            for factor in unique_factors[:5]:
                st.markdown(f"- {factor}")

        if not risk_factors and not priority_actions:
            st.success("✅ No major risk factors identified. Continue maintaining healthy habits!")
